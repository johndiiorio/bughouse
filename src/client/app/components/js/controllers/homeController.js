app.controller('homeController', ($scope, $http, $route, $window, $location) => {
	const socket = io('/lobby');
	const socketLoading = io('/loading');
	...
	$(document).ready(() => {
		...
		$('#minutesSlider').slider().on('slide', ev => {
			$('#minutesDisplay').text(`Minutes: ${ev.value}`);
		});
		$('#incrementSlider').slider().on('slide', ev => {
			$('#incrementDisplay').text(`Increment (seconds): ${ev.value}`);
		});
		$('#ratingSlider').slider().on('slide', ev => {
	...
	$scope.formatPlayer = function (player, game) {
		if (typeof player !== 'undefined') {
			if (typeof player[0] !== 'undefined') {
				let returnString = player[0].username;
				if (game.minutes < 3) {
	...
	$scope.getGamesForUser = function () {
		$http({
			...
			for (let i = 0; i < data.length; i++) {
				const minRange = parseInt(data[i].rating_range.substring(0, data[i].rating_range.indexOf(',')));
				const maxRange = parseInt(data[i].rating_range.substring(data[i].rating_range.indexOf(',') + 1));
				let passBool = false;
	...
	$scope.addPlayer = function (game) {
		...
		if (game.join_random) {
			const openSlots = $scope.getOpenSlots(game);
			...
			$http({
				...
				const putData = { player1: player1, player2: player2, player3: player3, player4: player4 };
				$http({
					method: 'PUT',
	...
	$scope.joinPlayer = function ($event) {
		...
		const slot = $event.target.id.substring($event.target.id.length - 1);
		let player1,
			player2,
			player3,
			player4;
	...
	$scope.userLogIn = function () {
		const user = { username: $scope.login.username, password: $scope.login.password };
		$http({
			method: 'POST',
			url: '/api/login',
			data: user
